...
export default function InvoiceFilters({ paymentModes, total, filtered }: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [isPending, startTransition] = useTransition();

  // Local state for search (debounced) — no useEffect sync to avoid overwriting mid-type
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex flex-col gap-1 min-w-50 flex-1">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Invoice no., customer, place of supply or additional info..."
              value={searchText}
